...
## prigh-specific notes

Use the [prigh] opam switch (vanilla OCaml 5.3, Jane Street v0.17, eio,
cohttp-eio, tls-eio, jsonaf, ocamlformat). opam lives at ~/.local/bin/opam;
if it is not on PATH: [export PATH=~/.local/bin:$PATH; eval $(opam env --switch=prigh)].
...
The bench section above is inherited from oxsat and does not apply here.
